...
const Swipe = () => {
    ...
    //gets the pics associated with the user
    const getPics = async (user) =>{
        try{
            let response = await fetch(`http://localhost:3001/get_images?username=${user}`)
            ...
            setImages(imgs)
            console.log(imgs)
            console.log(`Got images realted to ${user}`)
            if(imgs.length === 6){
                setIsLoaded(true)
            }
        }catch(error){
    ...
    //gets all the info need to populate of the profiles of the current user will be swiping on
    const getInfo = async (username, where) =>{
        try{
            ...
            if(where === 1){
                ...
                console.log('Abt to get info of the swipe subject')
    ...
    //gets a list of the users the the current user can swipe on that matches the filter specifications
    const getUsers = async () =>{
        try{
            let response = await fetch(`http://localhost:3001/get_users?table=${info['preference']}&sex=${localStorage.getItem('selectedSex')}&race=${localStorage.getItem('selectedRace')}&blood=${localStorage.getItem('selectedBlood')}&hair=${localStorage.getItem('selectedHair')}&eye=${localStorage.getItem('selectedEye')}&education=${localStorage.getItem('selectedEducation')}&distance=${localStorage.getItem('maxDistance')}&latitude=${info['latitude']}&longitude=${info['longitude']}&age=${localStorage.getItem('maxAge')}&ft=${localStorage.getItem('selectedFt')}&inches=${localStorage.getItem('selectedIn')}`)
            response = await response.json()

            //if there are no users available to look at, automatic return occurs
            ...
            let uniqueUsers = new Set([...users].filter(item => !liked.has(item)))
            ...
            uniqueUsers = new Set([...uniqueUsers].filter(item => item !== currentUser.username))
    ...
    //handles user actions, like swiping right or left on a user, and allows next user to pop up
    const dislike_like_user = async (e, like) =>{
        ...
        try{
            ...
            if(users.length == 0){
                ...
                let response = await fetch(`http://localhost:3001/update_likes_dislikes?table=${endpoint}&username=${username}&user=${user}`,{
                    method: 'POST',
